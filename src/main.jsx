...
import {
  AudioLines, ArrowDown, Check, ChevronDown, CircleHelp, Clock3, Copy, FileAudio,
  FolderOpen, GitCompareArrows, Headphones, History, Info, Languages,
  Link2, ListFilter, LoaderCircle, Logs, Menu, Minus, Pause, Pencil, Play, Plus,
...
function App() {
  ...
  const handleTimelineScroll = event => {
    ...
    // If the user scrolls up more than ~24px, treat that as "I want to look
    // at something above"; turn follow off so the next chunk doesn't yank
    // them back down.  Scrolling back to the bottom re-enables follow.
    if (distanceFromBottom > 24) setTimelineFollow(false)
  }
  useEffect(() => {
    if (!timelineFollow) return
  ...
  }, [leftItems, rightItems, timelineFollow])
  ...
  return (
    <div className="app-shell">
      ...
      <main className="main-content">
        ...
        <section className="comparison-panel"><div className="panel-heading"><div><div className="section-kicker"><span className="kicker-line" /> TIMELINE OUTPUT</div><h2>结果时间轴</h2></div><div className="panel-tools"><div className="search-box"><Search size={15} /><input value={query} onChange={event => setQuery(event.target.value)} placeholder="搜索转录或翻译…" /></div><button className="small-tool"><ListFilter size={15} /> 筛选</button><button className="small-tool icon-only"><SlidersHorizontal size={15} /></button></div></div><div className="timeline-header"><div className="group left"><span className="system-badge cyan">A</span><span className="label">{slotService('A')?.label || '系统 A'}</span><span className="url">{slotService('A')?.url || '未配置'}</span><span className="lat">ASR end</span></div><div className="spacer" /><div className="axis-label">ABSOLUTE ASR END TIME</div><div className="spacer" /><div className="group right"><span className="lat">ASR end</span><span className="url">{slotService('B')?.url || '未配置'}</span><span className="label">{slotService('B')?.label || '系统 B'}</span><span className="system-badge violet">B</span></div></div><div ref={timelineListRef} className="timeline-list absolute-timeline" onScroll={handleTimelineScroll}><div className="absolute-axis" /><div className="timeline-list-follow" style={{ position: 'absolute', top: 8, left: 8 }}><button type="button" className={`timeline-list-follow ${timelineFollow ? 'active' : ''}`} onClick={() => setTimelineFollow(value => !value)}><ArrowDown size={11} /> {timelineFollow ? 'FOLLOW ON' : 'FOLLOW OFF'}</button></div><div style={{ position: 'relative', height: `${timelineLayout.height}px`, minHeight: '100%' }}>{timelineLayout.rows.map((row, index) => <TimelineEventRow key={row.id} row={row} isLast={index === timelineLayout.rows.length - 1} selectedChunk={selectedChunk} selectedSide={selectedSide} onSelect={(chunkId, side) => { setSelectedChunk(chunkId); setSelectedSide(side) }} style={{ '--row-top': `${row.top}px`, '--row-height': `${row.height}px` }} />)}</div></div>{timelineLayout.rows.length === 0 && <div className="empty-search">没有找到匹配结果</div>}<div className="timeline-footer"><span><span className="footer-dot cyan" /> A · {leftItems.length} chunks</span><span><span className="footer-dot violet" /> B · {rightItems.length} chunks</span><span className="footer-note"><Info size={13} /> 当前按绝对 ASR 结束时间排布；左右结果各自落在自己的时间点上</span></div></section>
